...
def make_hiro_env(dist, arm="right"):
    base_extent = 5.0
    X_DIST = dist
...
def world_pose_to_robot_frame(robot, pose, baselink = BASE_LINK):
    trans, orient = pose
    robot_frame = get_link_pose(robot, link_from_name(robot, "panda_link0"))
    new_quat  = multiply_quats(multiply_quats(orient, robot_frame[1]), quaternion_inverse(robot_frame[1]))
    new_pose = (
        trans[0] - robot_frame[0][0],
