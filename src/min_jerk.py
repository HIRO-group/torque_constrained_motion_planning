...
from .panda_utils import TIME_STEP
import numexpr
...
def min_jerk(pos=None, dur=None, vel=None, acc=None, psg=None):
    ...
    if psg is None:					# passage times unknown, optimize
        if N > 2:
            ...
            psg = np.multiply(.5, psg)
            func = lambda psg_: mjCOST(psg_, pos, vel, acc, t0)
            psg = psg
            psg = scipy.optimize.fmin(func = func, x0 = psg, maxfun=750, ftol=1e-2)
        else:
            psg = []
    print(psg)
    print(len(psg))
...
def mjCOST(t, x, v0, a0, t0):
    ...
    j= numexpr.evaluate("""3*(3*aa0**2*T**4-2*aa0*aa1*T**4+3*aa1**2*T**4+24*aa0*T**3*vv0- \
